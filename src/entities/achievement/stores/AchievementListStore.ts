...
import { getAllAchievements, getPaginatedAchievements } from "../api";
import { Achievement } from "../types";
import SearchParamsModel, { UpdateUrlFn } from "entities/searchParams/model";

export class AchievementListStore {
  ...
  async fetchAchievements(): Promise<LoadResponse> {
    ...
    try {
      ...
      const { data, pagination } = await getPaginatedAchievements(
        page,
