...
import { getAllGrants, getPaginatedGrants } from "../api";
import { Grant } from "../types";
import SearchParamsModel, { UpdateUrlFn } from "entities/searchParams/model";

export class GrantListStore {
  grants: Grant[] = [];
  ...
  async fetchGrants(): Promise<LoadResponse> {
    ...
    try {
      ...
      const { data, pagination } = await getPaginatedGrants(
        page,
